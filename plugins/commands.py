...
import os, string, logging, random, asyncio, time, datetime, re, sys, json, base64, traceback
...
async def start(client, message):
    ...
    if data.startswith("search_"):
        ...
        await auto_filter(client, query, message, reply_msg, True, spoll=(query, files, offset, total_results))
        return
    ...
    if data.startswith("sendfiles"):
        ...
        if settings['is_shortlink'] and not await db.has_premium_access(user):
            ...
            return
    user = message.from_user.id
    logger.info(f"[DEBUG START] Entering file_ branch | data='{data}', pre='{pre}', file_id='{file_id}'")
    files_ = await get_file_details(file_id)           
    logger.info(f"[DEBUG START] get_file_details('{file_id}') returned document: {bool(files_)}")
    if not files_:
        try:
            ...
            if STREAM_MODE == True:
                ...
                reply_markup = None
            logger.info(f"[DEBUG START] Immediately before calling send_file() (encoded fallback) for file_id='{file_id}'")
            try:
                msg = await send_file(
                    bot=client,
                    chat_id=message.from_user.id,
                ...
                logger.error(f"[DEBUG START] send_file() (encoded fallback) failed for file_id '{file_id}': {e}\n{traceback.format_exc()}")
                raise
            filetype = msg.media
    ...
    if STREAM_MODE == True:
        ...
        reply_markup = None
    logger.info(f"[DEBUG START] Immediately before calling send_file() for file_id='{file_id}'")
    try:
        msg = await send_file(
            bot=client,
            chat_id=message.from_user.id,
        ...
        logger.error(f"[DEBUG START] send_file() failed for file_id '{file_id}': {e}\n{traceback.format_exc()}")
        raise
    btn = [[InlineKeyboardButton("✅ ɢᴇᴛ ғɪʟᴇ ᴀɢᴀɪɴ ✅", callback_data=f'del#{file_id}')]]
    k = await msg.reply(text=f"<blockquote><b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b>\n\nᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ɪɴ <b><u>10 mins</u> 🫥 <i></b>(ᴅᴜᴇ ᴛᴏ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs)</i>.\n\n<b><i>ᴘʟᴇᴀsᴇ ғᴏʀᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ ʏᴏᴜʀ sᴀᴠᴇᴅ ᴍᴇssᴀɢᴇs ᴏʀ ᴀɴʏ ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ.</i></b></blockquote>")
    await asyncio.sleep(600)
